...
import {interval, of, throwError, timer} from 'rxjs/index';
import {mapTo, take} from 'rxjs/internal/operators';
...
describe('Marble testing basics', () => {
  ...
  describe('hot observable', () => {
    ...
    it('should emit 4 stars after 1000ms', () => {
      const source = of(1,2,3,4,5,6,7).pipe(mapTo('*'), take(4));

      const expected = cold('(aaaa|)', {a: '*'});
      expect(source).toBeObservable(expected);
    });
